...
async def get_dashboard(
    ...
    logger.info(
        "business.tool.get_dashboard.response status_code=%s body=%s",
    ...
    if not response.content:
        logger.info("business.tool.get_dashboard.result data={}")
        return {}

    envelope = response.json()
    result = envelope.get("data", envelope) if isinstance(envelope, dict) else envelope
    logger.info("business.tool.get_dashboard.result data=%s", _short_text(str(result)))
    return result
